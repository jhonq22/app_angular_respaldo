...
@Component({
  selector: 'app-vacunas-perdidas-registrador-list',
  templateUrl: './vacunas-perdidas-registrador-list.component.html',
  styleUrls: ['./vacunas-perdidas-registrador-list.component.css']
})
export class VacunasPerdidasRegistradorListComponent implements OnInit {
  ...
  getVacunaPerdidas()
  {
    ...
    this.vacunasService.getVacunaPerdidaCentroSalud(params.id).subscribe(res => {
      this.vacunas = res;
  ...
  {
    const params = this.activatedRoute.snapshot.params;
    this.vacunasService.getVacunaPerdidaCentroSaludExcel(params.id).subscribe(res => {
    ...
    this.vacunas.forEach(element => {
      ...
      rows.push(temp);
       ...
       doc.autoTable(col, rows, 
        ...
        doc.save('Reporte_vacunas_perdidas_centro_salud.pdf');
